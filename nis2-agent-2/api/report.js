...
Die gapAnalysis MUSS alle 10 Bereiche enthalten.`;
  ...
  try {
    ...
    if (!res.ok) return new Response(JSON.stringify({ error: "KI nicht verfügbar" }), { status: 502, headers: { ...cors, "Content-Type": "application/json; charset=utf-8" } });
    ...
    return new Response(JSON.stringify({ error: "Interner Fehler" }), { status: 500, headers: { ...cors, "Content-Type": "application/json; charset=utf-8" } });
  }
}
