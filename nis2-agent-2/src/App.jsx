...
export default function App() {
  ...
  const erstelleReport = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error);
      const parsed = safeParse(data.result);
      if (!parsed) throw new Error("Report konnte nicht erstellt werden");
      setReport(parsed);
    } catch (e) { setError(e.message); }
      ...
      <main style={{ maxWidth: 880, margin: "0 auto", padding: "36px 18px 80px" }}>
        ...
        {page === "evidence" && (
          <div className="fade">
            ...
            <p style={{ fontSize: 14, color: C.muted, marginBottom: 24 }}>Laden Sie vorhandene Dokumente hoch. Die KI prüft kritisch, ob sie als Nachweis taugen. <button onClick={() => setPage("anleitung")} style={{ color: C.accent, background: "none", border: "none", cursor: "pointer", fontSize: 14, textDecoration: "underline" }}>Welche Dokumente?</button></p>
            ...
            <div style={{ ...card, marginBottom: 20 }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {DOCUMENT_GUIDE.map(doc => {
                  const g = geprüfteTypen.has(doc.name);
                  return <div key={doc.id} style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 12px", borderRadius: 20, fontSize: 12, border: `1px solid ${g ? C.green : C.borderLight}`, background: g ? C.greenDim : "transparent", color: g ? C.green : C.muted }}><span>{g ? "✓" : doc.icon}</span>{doc.name}</div>;
                })}
              </div>
            </div>
            ...
            <div style={{ ...card, marginBottom: 24 }}>
              ...
              <select style={{ ...inp, marginBottom: 16 }} value={pendingType} onChange={e => setPendingType(e.target.value)}>{DOCUMENT_GUIDE.map(d => <option key={d.id}>{d.name}</option>)}<option>Sonstiges</option></select>
              <div onClick={() => !uploading && fileRef.current?.click()} onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); handleFile(e.dataTransfer.files[0]); }} style={{ border: `2px dashed ${C.borderLight}`, borderRadius: 10, padding: "36px 24px", textAlign: "center", cursor: uploading ? "default" : "pointer", background: C.bg }}>
                <input ref={fileRef} type="file" accept=".pdf,.docx,.txt,.csv,.md" style={{ display: "none" }} onChange={e => handleFile(e.target.files[0])} />
                {uploading ? <div><div style={{ width: 32, height: 32, border: `3px solid ${C.border}`, borderTop: `3px solid ${C.accent}`, borderRadius: "50%", animation: "spin 0.8s linear infinite", margin: "0 auto 12px" }} /><div style={{ fontSize: 13, color: C.mutedLight, fontFamily: MONO }}>{uploadStatus}</div></div>
                : <div><div style={{ fontSize: 28, marginBottom: 8 }}>📄</div><div style={{ fontSize: 14, fontWeight: 500, marginBottom: 4 }}>Datei hierher ziehen oder klicken</div><div style={{ fontSize: 12, color: C.muted, fontFamily: MONO }}>PDF · Word · Text</div></div>}
              </div>
            </div>
            ...
            {evidences.length > 0 && <div style={{ display: "flex", flexDirection: "column", gap: 16, marginBottom: 24 }}>{evidences.map((ev, i) => <EvidenceCard key={i} ev={ev} plan={plan} onUpgrade={goUpgrade} />)}</div>}

            {doneEvidences.length > 0 && <button onClick={() => setPage("dashboard")} style={btn("primary", { width: "100%", padding: "13px" })}>Weiter zu Schritt 3: Report erstellen →</button>}
          </div>
        )}
