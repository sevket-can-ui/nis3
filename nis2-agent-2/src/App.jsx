          ...
          <div className="fade">
            <div style={{ marginBottom: 36 }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: 8, fontFamily: MONO, fontSize: 11, color: C.accent, marginBottom: 18, border: `1px solid ${C.accent}30`, borderRadius: 20, padding: "5px 12px" }}>
                <span style={{ width: 6, height: 6, borderRadius: "50%", background: C.green }} /> NIS2-READINESS-CHECK · STAND BSIG DEZ. 2025
              </div>
              <h1 style={{ fontSize: "clamp(27px,5vw,42px)", fontWeight: 700, letterSpacing: "-1.5px", lineHeight: 1.1, marginBottom: 16 }}>
                NIS2 verstehen,<br />bevor es teuer wird.
              </h1>
              <p style={{ fontSize: 16, color: C.mutedLight, lineHeight: 1.7, maxWidth: 620, marginBottom: 14 }}>
                Der NIS2 Agent prüft Ihre Betroffenheit, bewertet Ihre Nachweise und zeigt Ihre Lücken — strukturiert, verständlich und mit Bezug zum aktuellen BSIG. Der einfache erste Schritt für Unternehmen ohne eigene Compliance-Abteilung.
              </p>
              <p style={{ fontSize: 15, color: C.text, fontWeight: 600, marginBottom: 28 }}>
                Erst Klarheit. Dann gezielt investieren.
              </p>
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                <button onClick={() => setPage("profil")} style={btn("primary", { padding: "14px 26px", fontSize: 15 })}>Kostenlosen NIS2-Check starten</button>
              ...
              <p style={{ fontSize: 12, color: C.muted, marginTop: 14 }}>
                Keine Rechtsberatung. Keine offizielle BSI-Einstufung. Ein klarer Vorbefund, der Sie weiterbringt.
              </p>
            </div>

            {/* Klarheit vor Kosten — Einordnung */}
            <div style={{ ...card, marginBottom: 24, borderColor: `${C.accent}40` }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: C.muted, textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: 14 }}>Klarheit vor Kosten</div>
              <div className="grid3">
                {[
                  { t: "Bevor Sie nichts tun", d: "NIS2 gilt seit Dezember 2025 ohne Übergangsfrist. Wegsehen macht das Risiko nicht kleiner — nur unsichtbar." },
                  { t: "Bevor Sie Beratung kaufen", d: "Ein Beratungsprojekt kostet schnell mehrere tausend Euro. Klären Sie zuerst, wo Sie überhaupt stehen." },
                  { t: "Bevor Sie Tools einführen", d: "Erst wenn die Lücken bekannt sind, lohnt sich die Investition in Software oder interne Projekte." },
                ].map(m => (
                  <div key={m.t}>
                    <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 6 }}>{m.t}</div>
        ...
        {page === "dashboard" && (
          <div className="fade">
            ...
            {!report ? (
              ...
              <ReportView report={report} profil={profil} betroffenheit={betroffenheit} evidences={doneEvidences} plan={plan} onUpgrade={goUpgrade} onReset={() => setReport(null)} onSave={speichern} saveMsg={saveMsg} user={user} />
            )}
          </div>
        )}
...
// ─── Report View ───
function ReportView({ report, profil, betroffenheit, evidences, plan, onUpgrade, onReset, onSave, saveMsg, user }) {
      ...
      <div style={{ ...card, marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          {can(plan, "canExportPdf")
            ? <button onClick={() => generateReportPDF({ report, profil, betroffenheit, evidences })} style={btn("primary")}>📄 Als PDF herunterladen</button>
