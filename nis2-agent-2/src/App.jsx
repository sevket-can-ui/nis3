...
import { IMPRESSUM, DATENSCHUTZ, AGB } from "./legal";
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: SANS }}>
      ...
      {/* ════ FOOTER ════ */}
      <footer style={{ borderTop: `1px solid ${C.border}`, marginTop: 60, padding: "28px 24px", maxWidth: 1100, margin: "60px auto 0", display: "flex", flexWrap: "wrap", gap: 18, alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ fontSize: 12, color: C.muted }}>© {new Date().getFullYear()} NIS2 Agent</div>
        <div style={{ display: "flex", gap: 18, flexWrap: "wrap" }}>
          {[["impressum", "Impressum"], ["datenschutz", "Datenschutz"], ["agb", "AGB"]].map(([k, l]) => (
            <button key={k} onClick={() => { setPage(k); window.scrollTo(0, 0); }} style={{ background: "none", border: "none", color: C.muted, fontSize: 12, cursor: "pointer", fontFamily: SANS, textDecoration: "underline" }}>{l}</button>
          ))}
        </div>
      </footer>
    </div>
